...
void AShooterWeapon_Projectile::ImGuiDebug()
{
	Super::ImGuiDebug();
	if(!MyPawn||!MyPawn->Candebug()||MyPawn->GetWeapon()!=this) return;
	ImGui::Text("ProjectileLife:%f",ProjectileConfig.ProjectileLife);
	ImGui::SliderFloat("ProjectileLife",&ProjectileConfig.ProjectileLife,0,20);
	ImGui::Text("ExplosionRadius:%f",ProjectileConfig.ExplosionRadius);
	ImGui::SliderFloat("ExplosionRadius",&ProjectileConfig.ExplosionRadius,0,1000);
	ImGui::Text("ExplosionDamage:%f",ProjectileConfig.ExplosionDamage);
	ImGui::SliderInt("ExplosionDamage",&ProjectileConfig.ExplosionDamage,0,1000);
	if(HasAuthority())
	{
		/**/		
	}
