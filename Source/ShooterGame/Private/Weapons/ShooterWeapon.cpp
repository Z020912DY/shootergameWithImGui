...
void AShooterWeapon::ServerImGuiDebug_Implementation(int32 CurrentAmmo,int32 CurrentAmmoInClip)
{
...
void AShooterWeapon::ImGuiDebug()
{
	if(!MyPawn||!MyPawn->Candebug()||MyPawn->GetWeapon()!=this) return;
	ImGui::Text("Common Basic Attribute");
	ImGui::SliderInt("Current Ammo in Clip",&CurrentAmmoInClip,0,1000);
	ImGui::SliderInt("Current total Ammo",&CurrentAmmo,0,1000);
	ImGui::SliderInt("AmmoPerClip",&WeaponConfig.AmmoPerClip,0,1000);
	ImGui::Text("MaxAmmo:%d",GetMaxAmmo());
	ImGui::SliderInt("MaxAmmo",&WeaponConfig.MaxAmmo,0,1000);
	ImGui::Text("InitialClips:%d",WeaponConfig.InitialClips);
